...
module.exports = {
  ...
  onStart: async ({ api, args, event, commandName }) => {
    ...
    api.sendMessage({
      body: msg + "Reply to this message with a number to download the song.",
  ...
  onReply: async ({ event, api, Reply }) => {
    try {
      ...
      if (!isNaN(choice) && choice > 0 && choice <= result.length) {
        ...
        const response = await axios.get(`${await baseApiUrl()}/ytDl3?link=${idvideo}&format=mp3`);
      ...
      console.error(error);
      api.sendMessage("⭕ Sorry, an error occurred. Audio might be too large or failed to download.", event.threadID, event.messageID);
    }
  }
};
...
// Function to download thumbnail as stream
